...
class CarAgencyManager {
  ...
  transferCarBetweenAgencies(fromAgencyId, toAgencyId, carId) {
    ...
    if (car) {
      ...
      console.log("The car has been transferred successfully");
      return true;
    } else {
      console.log("The car has not transferred ");
      return false;
    }
  }
}
...
class CustomerManager {
  ...
  changeCustomerCash(customerId, cash) {
    return customers
      ...
      .find((customer) => customer !== undefined || false);
  }
  ...
  getCustomerTotalCarValue(customerId) {
    ...
    if (customer) {
      ...
      customer.cars.forEach((car) => {
        totalValue = totalValue + car.price;
      });
      console.log("Total value calculated successfully");
      return totalValue;
    } else {
...
class CarManager {
  ...
  searchCars(year, price, brand) {
    return this.agencies
      .flatMap((agency) => agency.cars.flatMap((model) => model.models))
      .filter((car) => {
        if (year) {
  ...
  getMostExpensiveCar() {
    return this.agencies
     .flatMap((agency) => agency.cars.flatMap((model) => model.models))
     .sort((car1, car2) => car2.price - car1.price)[0];
  ...
  getCheapestCar() {
    return this.agencies
     .flatMap((agency) => agency.cars.flatMap((model) => model.models))
     .sort((car1, car2) => car1.price - car2.price)[0];
...
class CarPurchaseManager {
  ...
  sellCar(carId, customerId) {
     const customer = this.customers.searchCustomer(customerId);
    const car = this.agencies
     .searchAgency(car.ownerId)
     .cars.find((car) => car.models.find((model) => model.carNumber === carId));
     const agency = this.agencies.searchAgency(car.ownerId);
     const carModel = agency.cars.find((car) => car.models.find((model) => model.carNumber === carId));
     const carPrice = carModel.models.find((model) => model.carNumber === carId).price;
     const customerCash = customer.cash;
     const agencyCash = agency.cash;
     const agencyTax = carPrice * 0.1;
     const customerTax = carPrice * 0.05;
     const customerNewCash = customerCash - (carPrice + customerTax);
     const agencyNewCash = agencyCash + (carPrice - agencyTax);
     const agencyNewTax = agencyTax + this.taxesAuthority.totalTaxesPaid;
     const customerNewTax = customerTax + this.taxesAuthority.totalTaxesPaid;
     const totalTaxesPaid = agencyTax + customerTax;
     const sumOfAllTransactions = totalTaxesPaid + this.taxesAuthority.sumOfAllTransactions;
     const numberOfTransactions = this.taxesAuthority.numberOfTransactions + 1;
     this.taxesAuthority.totalTaxesPaid = totalTaxesPaid;
     this.taxesAuthority.sumOfAllTransactions = sumOfAllTransactions;
     this.taxesAuthority.numberOfTransactions = numberOfTransactions;
     if (customer && car) {
       this.customers.changeCustomerCash(customerId, customerNewCash);
       this.agencies.changeAgencyCash(car.ownerId, agencyNewCash);
       this.agencies.removeCarFromAgency(car.ownerId, carId);
       this.agencies.addCarToAgency(customerId, car);
       console.log("The car has been sold successfully");
       return true;
     } else {
...
// );
// console.log("Search Agency by Name: ", carAgencyManager.searchAgency("CarMax"));
// console.log(
...
//   carAgencyManager.removeCarFromAgency("Plyq5M5AZ", "AZJZ4")
// );
// console.log(
...
// console.log(
//   "Change Agency Cash: ",
//   carAgencyManager.changeAgencyCashOrCredit("Plyq5M5AZ", 500000)
// );
// console.log(
//   "Change Agency Cash: ",
//   carAgencyManager.changeAgencyCashOrCredit("Plyq5M5AZ", "invalid", 5000000)
// );
// console.log(
//   "Change Agency Cash (Invalid): ",
//   carAgencyManager.changeAgencyCashOrCredit("Plyq5M5AZ", "invalid", "invalid")
// );
