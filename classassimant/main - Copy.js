...
class CarAgencyManager {
  ...
  searchAgency(idOrName) {
    if (typeof idOrName === "string") {
      const foundAgency = this.agencies.find(
        (agency) => agency.agencyId === idOrName
      );
      if (foundAgency) {
        return foundAgency;
      } else {
        const foundAgency = this.agencies.find(
          (agency) => agency.agencyName === idOrName
        );
        if (foundAgency) {
          return foundAgency;
        }
  ...
  addCarToAgency(agencyId, car) {
    ...
    const brandsArr = agency.cars;
    const brand = brandsArr.find((brnd) => brnd.brand === car.brand);
    if (brand) {
      brand.models.push(car.models[0]);
    } else {
      brandsArr.push(car);
    }
    return true;
  ...
  #findCar(agencyId, carNumber) {
    ...
    if (agency) {
      ...
      for (let brand of brandsArr) {
        const i = brand.models.findIndex((car) => car.carNumber === carNumber);
        if (i !== -1) {
          carInfo.brand = brand;
          carInfo.brandArrIndex = i;
          car = carInfo;
          break;
        }
      }
  ...
  // @param {string} agencyId - The ID of the agency
  // @param {string} carNumber - The number of the car to be removed
  // @return {boolean} - true if removed successfully, false otherwise
  removeCarFromAgency(agencyId, carNumber) {
    const car = this.#findCar(agencyId, carNumber);
    if (car) {
      car.brand.models.splice(car.brandArrIndex, 1);
  ...
  changeAgencyCashOrCredit(agencyId, cash = "invalid") {
    if (cash === "invalid") {
      return false;
    }
    const agency = this.searchAgency(agencyId);
    if (agency) {
      ...
      if (cash <= agency.cash) {
        agency.cash -= cash;
      } else if (cash <= agency.cash + agency.credit) {
        agency.credit = agency.cash + agency.credit - cash;
        agency.cash = 0;
      } else {
        console.log(agency);
        return false;
  ...
  // @param {string} agencyId - The ID of the agency
  // @param {string} carNumber - The number of the car
  // @param {number} newPrice - The new price of the car
  // @return {boolean} - true if updated successfully, false otherwise
  updateCarPrice(agencyId, carNumber, newPrice) {
    const car = this.#findCar(agencyId, carNumber);
    if (car) {
      // console.log(".......... before updateCarPrice");
      // console.log(car.brand.models[car.brandArrIndex]);
      car.brand.models[car.brandArrIndex].price = newPrice;
      // console.log("after updateCarPrice");
      // console.log(car.brand.models[car.brandArrIndex]);
      return true;
    }
    return false;
